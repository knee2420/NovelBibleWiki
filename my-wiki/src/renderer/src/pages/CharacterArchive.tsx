 ...
 useEffect(() => {
  ...
  return (
    <div className="min-h-screen animate-in fade-in duration-500">
      ...
      {/* ▼ 2. 캐릭터 카드 리스트 (Settings 컴포넌트 제거됨) */}
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
        {characters.map((char) => (
          <CharacterCard key={char.id} data={char} onClick={setSelectedCharId} />
        ))}
      </div>

      {/* ▼ 3. 상세 보기 모달 */}
      {selectedCharacter && (
        <CharacterDetail data={selectedCharacter} onClose={() => setSelectedCharId(null)} />
      )}
    </div>
  )
}
