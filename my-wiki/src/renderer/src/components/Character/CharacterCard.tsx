    ...
    >
      ...
      <div className="relative aspect-[3/4] w-full overflow-hidden rounded-xl bg-[#1e232e] border-2 border-[#2d3243] shadow-lg group-hover:border-cyan-500/50 group-hover:shadow-[0_0_20px_rgba(6,182,212,0.3)]">
        ...
        <div className="absolute top-3 left-3 z-10">
          <div className="flex h-8 w-8 items-center justify-center rounded-full bg-slate-900 border border-slate-600 shadow-md group-hover:border-cyan-400 group-hover:bg-cyan-950 transition-colors">
        ...
        <div className="absolute inset-0 flex flex-col items-center p-4">
          <div className="flex-1 flex items-center justify-center w-full relative mt-2">
            {/* 광원 효과 */}
            <div className="absolute w-32 h-32 bg-cyan-500/10 rounded-full blur-xl group-hover:bg-cyan-500/20 transition-all" />

            {/* 이미지 프레임 (크기 확대: w-32 ~ w-36) */}
            <div className="relative w-32 h-32 md:w-36 md:h-36 rounded-full border-4 border-[#373e4e] overflow-hidden shadow-inner bg-slate-800 group-hover:border-cyan-500/30 transition-colors">
              {data.image ? (
                <img
                  src={data.image}
                  alt={data.name}
          ...
          <div className="h-20 flex flex-col items-center justify-center w-full border-t border-slate-700/50 pt-2 mt-2">
            <h3 className="text-lg font-bold text-slate-100 group-hover:text-cyan-400 transition-colors text-center truncate w-full px-2">
              {data.name}
            </h3>
